...
st.caption(
    "Stay updated with live, upcoming, and past matches—all in one place! ⚽📅")
st.divider()
...
for i, (league_name, league_code) in enumerate(league_codes.items()):
    with tabs[i]:
        ...
        with col1:
            current_season = int(
                competition_data["currentSeason"]["startDate"][:4])
            current_matchday = int(
                competition_data["currentSeason"]["currentMatchday"])
        ...
        if not match_data or "matches" not in match_data:
            st.warning(
                f"⚠️ No matches available for the {selected_season} season.")
        ...
        for match in match_data["matches"]:
            ...
            home_score = score["fullTime"]["home"] if score["fullTime"]["home"] is not None else "-"
            away_score = score["fullTime"]["away"] if score["fullTime"]["away"] is not None else "-"
            ...
            with col1:
                st.markdown(
                    f'<div style="{center_style}"><img src="{home_team["crest"]}" width="50" style="margin-bottom: 10px;"><br><strong>{home_team["shortName"]}</strong></div>', unsafe_allow_html=True)

            with col2:
                if match_status == "FINISHED":
                    st.markdown(
                        f'<div style="{center_style}"><h3>{match_result}</h3></div>', unsafe_allow_html=True)
                else:
                    st.markdown(
                        f'<div style="{center_style}"><strong>Status:</strong> {match_status}</div>', unsafe_allow_html=True)

            with col3:
                st.markdown(
                    f'<div style="{center_style}"><img src="{away_team["crest"]}" width="50" style="margin-bottom: 10px;"><br><strong>{away_team["shortName"]}</strong></div>', unsafe_allow_html=True)
